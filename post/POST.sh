    ...
    for file in $(ls _params)
        ...
        do
            ...
            if [ $is -eq 0 ]; then
                ...
                local key=${file%.txt}
                local header=$(head -n 1 "_params/${file}")

                if [[ $header == "__TYPE@RANDOMNUMBER__" ]]; then
                    local min=$(sed "2q;d" "_params/${file}")
                    local max=$(sed "3q;d" "_params/${file}")
                    local diff=$((max - min + 1))
                    local value=$(( ($RANDOM % diff) + min ))


                else
                    local n_of_lines=$(wc -l < "_params/${file}")
                    local f_line=$(( ($RANDOM % n_of_lines) + 1  ))
                    local value=$(sed "${f_line}q;d" "./_params/${file}")
                fi
...
if [ $VERBOSE -eq 0 ]; then
    ...
    echo $(curl -v -X POST $URL -H $HEADERS -d \'$data\')
